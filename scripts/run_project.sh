...
kedro run || err "Fallo en kedro run (continuo con los siguientes pasos)"

# 4) Levantar Kedro Viz (en background)
log "Levantando Kedro Viz en http://localhost:${KEDRO_VIZ_PORT}"
nohup kedro viz --no-browser --host="$HOST" --port="$KEDRO_VIZ_PORT" >/tmp/kedro_viz.log 2>&1 &
VIZ_PID=$!
...
nohup kedro jupyter notebook --NotebookApp.token='' --NotebookApp.password='' --port="$JUPYTER_PORT" --ip="$HOST" >/tmp/jupyter.log 2>&1 &
JUPYTER_PID=$!
